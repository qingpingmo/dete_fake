    ...
    def setup(self, stage=None):
        ...
        if stage == "fit" or stage is None:
            if self.dataset == "SIMBA":
                mass_mstar = np.load(
                    'Grids_Mcdm_SIMBA_CV_128_z=0.0.npy'
                )
                mass_cdm = np.load(
                    'Grids_Mcdm_SIMBA_CV_128_z=1.0.npy'
                )
            elif self.dataset == "Astrid":
                mass_mstar = np.load(
                    'Grids_Mcdm_Astrid_CV_128_z=0.0.npy'
                )
                mass_cdm = np.load(
                    'Grids_Mcdm_Astrid_CV_128_z=1.0.npy'
                )
            elif self.dataset=="IllustrisTNG":
                mass_mstar = np.load(
                    'Grids_Mcdm_IllustrisTNG_CV_128_z=0.0.npy'
                )
                mass_cdm = np.load(
                    'Grids_Mcdm_IllustrisTNG_CV_128_z=1.0.npy'
                )
        ...
        if stage == "test" or stage is None:
            mass_mstar = np.load(
                'Grids_Mcdm_IllustrisTNG_1P_128_z=0.0.npy'
            )
            mass_cdm = np.load(
                'Grids_Mcdm_IllustrisTNG_1P_128_z=1.0.npy'
            )
