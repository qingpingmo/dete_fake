...
def draw_figure(x, sample, conditioning, dataset):
    ...
    ax.flat[0].imshow(conditioning[0, 0, slice_idx, :, :].squeeze().cpu(), cmap='copper', vmin=-.1, vmax=11)
    ax.flat[1].imshow(x[0, 0, slice_idx, :, :].squeeze().cpu(), cmap='cividis', vmin=-2, vmax=6)
    ax.flat[2].imshow(sample[0, 0, slice_idx, :, :].squeeze().cpu(), cmap='cividis', vmin=-2, vmax=6)
