...
def apply_dct(image, keep_ratio=0.1):
    ...
    h_keep = int(h * (1 - keep_ratio))  # 从高频开始保留的索引
    w_keep = int(w * (1 - keep_ratio))  # 从高频开始保留的索引
    # 创建一个全为0的数组，然后将高频部分的值填充进去
    dct_zeroed = np.zeros_like(dct_transformed)
    dct_zeroed[h_keep:, w_keep:] = dct_transformed[h_keep:, w_keep:]

    # 应用逆DCT变换
    idct_transformed = idct(idct(dct_zeroed, axis=0, norm='ortho'), axis=1, norm='ortho')
...
# 实例化ProGAN_Dataset
progan_dataset = ProGAN_Dataset(root_dir='/opt/data/private/rstao/code/DeepfakeDetection/datasets/CNNDetection/progan_train', transform=transform)
...
def main():
    # 实例化训练集
    train_dataset = ProGAN_Dataset(root_dir='/opt/data/private/rstao/code/DeepfakeDetection/datasets/CNNDetection/progan_train', transform=transform)
    
    # 实例化验证集
    val_dataset = ProGAN_Dataset(root_dir='/opt/data/private/rstao/code/DeepfakeDetection/datasets/CNNDetection/CNN_synth_testset', transform=transform)
