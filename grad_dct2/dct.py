...
class FusionCapsNet(nn.Module):
    def __init__(self):
        ...
        self.classifier = nn.Linear(262144, 2)  # 注意这里的维度需要和上一层的输出匹配

    def forward(self, x_dct):
        #print("Input shape:", x_dct.shape)  # 打印输入形状
        dct_features = self.dct_feature_extractor(x_dct)
        #print("After dct_feature_extractor shape:", dct_features.shape)  # 打印DCT特征提取后的形状

        cbam_features = self.cbam(dct_features)
        #print("After CBAM shape:", cbam_features.shape)  # 打印经过CBAM处理后的形状

        cbam_features = cbam_features.view(x_dct.size(0), -1)
        #print("Before classifier shape:", cbam_features.shape)  # 打印进入分类器之前的形状

        outputs = self.classifier(cbam_features)
        #print("Output shape:", outputs.shape)  # 打印输出形状
        return outputs
...
def train(trainloader, model, criterion, optimizer, epoch, scheduler=None):
    model.train()
    running_loss = 0.0
    correct = 0
    total = 0
    ...
    for i, (dct_images, labels) in enumerate(trainloader):
        ...
        if scheduler is not None and epoch < warm:  # Update the learning rate using the scheduler during the warm-up phase
            scheduler.step()
...
def validate(val_loader, model, criterion):
    ...
    total = 0  # 初始化总样本数为0
    all_predicted = []  # 用于存储所有的预测值
    all_targets = []  # 用于存储所有的真实标签

    with torch.no_grad():  # 在验证过程中不计算梯度
        for i, (dct_images, labels) in enumerate(val_loader):  # 遍历验证数据集
            dct_images, labels = dct_images.to(device), labels.to(device)  # 将数据和标签移动到指定的设备
            outputs = model(dct_images)  # 使用模型进行预测，注意这里只传入了DCT图像
            ...
            all_predicted.extend(predicted.cpu().numpy())  # 将预测的结果添加到列表中
            all_targets.extend(labels.cpu().numpy())  # 将真实标签添加到列表中

    val_loss /= len(val_loader)  # 计算平均验证损失
    acc = 100. * correct / total  # 计算验证集上的准确率
    ap = average_precision_score(all_targets, all_predicted)  # 计算平均精度分数（AP）
...
def main():
    ...
    # Train the model
        if epoch < warm:
            train_loss, train_acc = train(trainloader, net, loss_function, optimizer, epoch, warmup_scheduler)
        else:
            train_loss, train_acc = train(trainloader, net, loss_function, optimizer, epoch)
        # Update the learning rate if not in the warm-up phase
        if epoch >= warm:
