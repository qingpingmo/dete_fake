...
class CrossAttention(nn.Module):
    def __init__(self, channel, reduction_ratio=16):
        ...
        # Initialize ChannelAttention with 256 channels, correct the parameter name to 'in_planes'
        self.channel_attention = ChannelAttention(in_planes=256, ratio=reduction_ratio)
        self.spatial_attention = SpatialAttention()

    def forward(self, x1, x2):
        # Channel attention
        ca1 = self.channel_attention(x1)
        ca2 = self.channel_attention(x2)

        # Spatial attention
        sa1 = self.spatial_attention(x1)
        sa2 = self.spatial_attention(x2)

        # Fusion of features with attention
        fusion1 = ca1 * sa1 * x2
        fusion2 = ca2 * sa2 * x1
...
class FusionCapsNetWithFPN(nn.Module):
    def __init__(self):
        ...
        self.grad_feature_extractor = nn.Sequential(
         ...
         # Replace FPN with CrossAttention
        self.cross_attention = CrossAttention(channel=512)  # Assuming combined features have 512 channels

        # Classifier
        self.classifier = nn.Linear(256, 2)  # Adjust according to the final feature map size
    ...
    def forward(self, x_dct, x_grad):
        ...
        attention_features = self.cross_attention(dct_features, grad_features)

        # Global Average Pooling and classification
        pooled_features = F.adaptive_avg_pool2d(attention_features, (1, 1)).view(attention_features.size(0), -1)
