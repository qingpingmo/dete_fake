...
class FusionCapsNet(nn.Module):
    def __init__(self):
        ...
        self.capsule_layer = CapsuleLayer(
            ...
            in_channels=512, 
            out_channels=32
            # No need to specify kernel_size and stride here since they have default values in CapsuleLayer
        )
        self.feature_transform = nn.Linear(327680, 288000)
        self.classifier = nn.Linear(288000, 2)
        #print("Expected input features of self.classifier:", self.classifier.in_features)
    ...
    def forward(self, x_dct, x_grad):
        ...
        #print("Shape of capsule_output before view:", capsule_output.shape)
        capsule_output = self.feature_transform(capsule_output.view(x_dct.size(0), -1))
        #print("Shape of capsule_output after view:", capsule_output.shape)
