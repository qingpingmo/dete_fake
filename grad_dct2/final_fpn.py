...
class FPN(nn.Module):
    def __init__(self, input_channels, feature_sizes=[256, 256]):
        ...
        self.reduce_layers = nn.ModuleList([nn.Conv2d(in_channels, fs, kernel_size=1) for in_channels, fs in zip(input_channels, feature_sizes)])
        self.smooth_layers = nn.ModuleList([nn.Conv2d(fs, fs, kernel_size=3, padding=1) for fs in feature_sizes])
        self.upsample_layers = nn.ModuleList([nn.Upsample(scale_factor=2, mode='nearest') for _ in range(len(feature_sizes) - 1)])

        # Adaptive pooling layer to adjust spatial dimensions
        self.adaptive_pool = nn.AdaptiveAvgPool2d((32, 32))

    def forward(self, inputs):
        inputs = [reduce_layer(x) for x, reduce_layer in zip(inputs, self.reduce_layers)]
        ...
        features = [smooth_layer(x) for x, smooth_layer in zip(inputs, self.smooth_layers)]
        return features
...
class FusionCapsNetWithFPN(nn.Module):
    def __init__(self):
        ...
        # Classifier
        self.classifier = nn.Linear(256 * len(self.fpn.feature_sizes), 2)
    ...
    def forward(self, x_dct, x_grad):
        ...
        #print(f"dct_features shape: {dct_features.shape}")
        ...
        features = self.fpn([dct_features, grad_features])
        pooled_features = [self.global_avg_pool(feature) for feature in features]
        flat_features = torch.cat([feature.view(x_dct.size(0), -1) for feature in pooled_features], dim=1)
