...
class ImageProcessor(nn.Module):
    def __init__(self, embed_size=1, heads=1, dropout=0.1, forward_expansion=4):
        super(ImageProcessor, self).__init__()
        self.embed_size = embed_size  # 正确定义 embed_size
        self.dct_attention = TransformerBlock(embed_size, heads, dropout, forward_expansion)
        self.grad_attention = TransformerBlock(embed_size, heads, dropout, forward_expansion)

    def forward(self, image_dct, image_grad):
        C, H, W = image_dct.size()
        B=1
        # 确保 image_dct 和 image_grad 的通道数与 embed_size 匹配
        assert C == self.embed_size, "Channel size of image_dct must match embed_size"

        # 将图像展平
        image_dct_flat = image_dct.view(B, C, H * W).transpose(0, 2).transpose(1, 2)  # [H*W, B, C]
        image_grad_flat = image_grad.view(B, 1, H * W).transpose(0, 2).transpose(1, 2)  # [H*W, B, 1]

        # 应用注意力机制
        image_dct_att = self.dct_attention(image_dct_flat, image_dct_flat, image_dct_flat)
        image_grad_att = self.grad_attention(image_grad_flat, image_grad_flat, image_grad_flat)

        # 特征融合
        combined_feature = torch.cat((image_dct_att, image_grad_att), dim=2)
...
def generate_gradient_image(image):
    ...
    # Apply Sobel filters
    grad_x = np.abs(convolve2d(image_np, sobel_x, mode='same', boundary='wrap'))
    grad_y = np.abs(convolve2d(image_np, sobel_y, mode='same', boundary='wrap'))
    ...
    pil_image = Image.fromarray(gradient_uint8, 'L')
    return pil_image
...
def apply_dct(image, keep_ratio=0.1, embed_size=32):
    ...
    # Retain a certain percentage of DCT coefficients
...
class ProGAN_Dataset(Dataset):
    ...
    # Inside the __getitem__ method of ProGAN_Dataset
    def __getitem__(self, idx):
        img_path = self.image_paths[idx]
        image = Image.open(img_path).convert('RGB')

    # Apply DCT transformation first
        image_dct = apply_dct(image, embed_size=self.image_processor.embed_size)
    
    # Generate the gradient image
        image_grad = generate_gradient_image(image)

    # Then apply the standard transformation pipeline
        if self.transform:
            image_dct = self.transform(image_dct)
            image_grad = self.transform(image_grad)

    # Use ImageProcessor
        #print("Shape of image_dct:", image_dct.shape)
        ##print("Shape of image_grad:", image_grad.shape)
        processed_image = self.image_processor(image_dct, image_grad).detach()
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1):
        ...
        self.conv3 = nn.Conv2d(planes, planes, kernel_size=3, stride=1, padding=1, bias=False)  # 新增加的卷积层
    ...
    def forward(self, x):
        ...
        out = F.relu(self.bn3(self.conv3(out)))  # 添加额外的卷积层和激活函数
        out += self.shortcut(x)
        out = F.relu(out)
        return out
