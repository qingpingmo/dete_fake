...
class ProGAN_Dataset(Dataset):
    ...
    # Apply gradient image transform
    ...
    # Convert to numpy arrays
        image_dct_np = np.array(image_dct_gray)
        image_grad_np = np.array(image_grad_gray)

    # Combine the images side by side in a larger array
        height = max(image_dct_np.shape[0], image_grad_np.shape[0])
        width = image_dct_np.shape[1] + image_grad_np.shape[1]
        image_combined_np = np.zeros((height, width), dtype=np.uint8)

    # Place the images in the combined array
        image_combined_np[:image_dct_np.shape[0], :image_dct_np.shape[1]] = image_dct_np
        image_combined_np[:image_grad_np.shape[0], image_dct_np.shape[1]:] = image_grad_np

    # Convert back to PIL Image in 'L' mode (grayscale)
        image_combined = Image.fromarray(image_combined_np, mode='L')

        if self.transform:
            image_combined = self.transform(image_combined)
