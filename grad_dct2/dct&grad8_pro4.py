...
def find_least_varying_patch(image, patch_size=(256, 256)):
    min_variance = float('inf')
    best_patch = None

    for i in range(image.width - patch_size[0] + 1):
        for j in range(image.height - patch_size[1] + 1):
            patch = image.crop((i, j, i + patch_size[0], j + patch_size[1]))
            patch_np = np.array(patch)
            variance = patch_np.var()
...
class ProGAN_Dataset(Dataset):
    ...
    # 找到像素变化最小的256x256区域
        least_varying_patch = find_least_varying_patch(image)

    # 应用DCT变换
        dct_image = apply_dct(least_varying_patch)

    # 应用梯度变换
        grad_image = generate_gradient_image(least_varying_patch)
