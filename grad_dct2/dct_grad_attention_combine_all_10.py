...
# 在 ImageProcessor 的 forward 方法中传递通道数
class ImageProcessor(nn.Module):
    def __init__(self, channels, attention_heads=4):  
        super(ImageProcessor, self).__init__()
        embed_size = 32
        self.attention_spatial = SpatialAttention(channels)  # Use Spatial Attention Module


    def forward(self, image, channels):
        embed_size = 32  # Ensure consistent embed_size
        attention_heads = 1
        ...
        image_dct_flat = image_dct.view(B, C, H * W).transpose(1, 2)
        image_grad_flat = image_grad.view(B, 1, H * W).transpose(1, 2)

    # Print the shapes for debugging
        #print(f"image_dct_flat shape: {image_dct_flat.shape}")
        #print(f"image_grad_flat shape: {image_grad_flat.shape}")
    ...
    # Initialize variables
        ...
        # Ensure that the input tensor has the expected number of channels (32)
            #print(f"ImageProcessor input shape: {image_dct_flat.view(B, channels, H, W).shape}")  # 添加此行
            image_dct_att = self.attention_spatial(image_dct_flat.view(B, channels, H, W))
        
            image_grad_att = self.attention_spatial(image_grad_flat.view(B, 1, H, W))

        # Print the shapes after attention for debugging
            #print(f"image_dct_att shape: {image_dct_att.shape}")
            #print(f"image_grad_att shape: {image_grad_att.shape}")
        except Exception as e:
            #print(f"Error during attention module: {e}")
            pass

    # Check if variables were assigned before using them
        if image_dct_att is not None and image_grad_att is not None:
        # Feature fusion
            combined_feature = torch.cat((image_dct_att, image_grad_att), dim=1)
            return combined_feature.detach().clone()
        else:
        # Handle the case where attention module failed
            return torch.zeros_like(image_dct_flat).detach().clone()
...
# 在 SpatialAttention 的初始化方法中添加通道数参数
...
def apply_dct(image, keep_ratio=0.1):
    ...
    # 打印 dct_transformed 的形状
    #print(f"dct_transformed shape: {dct_transformed.shape}")
...
class ProGAN_Dataset(Dataset):
    def __init__(self, root_dir, transform=None):
        ...
        self.image_processor = ImageProcessor(channels=3)  # 传递通道数，这里假设是 RGB 图像，通道数为 3
    ...
    # 然后应用 ImageProcessor
        processed_image = self.image_processor(image, channels=image.size()[0]).detach()
...
class BasicBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.use_attention:
            spatial_attention = SpatialAttention(out.size(1))
...
def main():
    ...
    # 创建数据加载器
    trainloader = DataLoader(train_dataset, batch_size=4, shuffle=True, num_workers=1, pin_memory=True)
    valloader = DataLoader(val_dataset, batch_size=4, shuffle=False, num_workers=1, pin_memory=True)
