...
class ProGAN_Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return grad_image, new_feature_image, label
...
def train(trainloader, model, criterion, optimizer, epoch, scheduler, warm):
    ...
    for i, (grad_images, new_feature_images, labels) in enumerate(trainloader):
        grad_images, new_feature_images, labels = grad_images.to(device), new_feature_images.to(device), labels.to(device)
        optimizer.zero_grad()
        outputs = model(grad_images, new_feature_images)
...
def validate(val_loader, model, criterion):
    ...
    with torch.no_grad():  # Disabling gradient calculation
        for i, (grad_images, new_feature_images, labels) in enumerate(val_loader):
            grad_images, new_feature_images, labels = grad_images.to(device), new_feature_images.to(device), labels.to(device)
            outputs = model(grad_images, new_feature_images)  # Adjusted to take two inputs
            loss = criterion(outputs, labels)
