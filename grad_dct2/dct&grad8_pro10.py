...
def train(trainloader, model, criterion, optimizer, epoch, scheduler, warm):
        ...
        for b in range(batch_size):
            ...
            if b < len(dct_blocks_batch) and b < len(grad_blocks_batch):
                ...
                for block_idx in range(64):  # 假设每张图片被分成64个块
                    ...
                    if predicted == 0:  # 如果任何一个块被预测为假
                        correct += 0  # 整张图片被标记为假
                        break  # 不再处理后续的块
                else:  # 只有当所有块都被处理后（没有遇到break）
                    correct += 1  # 整张图片被标记为真
    ...
    train_loss = running_loss / (total * 64)  # 64是因为每张图片有8x8块
...
def validate(val_loader, model, criterion):
    ...
    with torch.no_grad():
        for i, (dct_blocks_batch, grad_blocks_batch, labels) in enumerate(val_loader):
            labels = labels.to(device)  # 确保标签在正确的设备上
            batch_size = len(labels)
