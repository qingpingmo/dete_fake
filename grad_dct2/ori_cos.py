...
class ProGAN_Dataset(Dataset):
    ...
    def _process_image(self, image):
        ...
        new_feature_image_array = np.zeros((height, width))

        for i in range(height - 1):  # Exclude the last row to avoid index out of bounds
            for j in range(width - 1):  # Exclude the last column
                distances = []
                for channel in range(channels):
                    pixel = image_array[i, j, channel]
                    neighbors = [
                        image_array[i, j+1, channel],    # Right
                        image_array[i+1, j, channel],    # Bottom
                        image_array[i+1, j+1, channel],  # Bottom right
                    ]
                    # Calculate Euclidean distance for each neighbor in the color space
                    distances += [np.linalg.norm(pixel - neighbor) for neighbor in neighbors]
                
                # Average distance across all channels and neighbors
                avg_distance = np.mean(distances)
                # Assign the average distance as the new pixel value for this position
                new_feature_image_array[i, j] = avg_distance

        # Normalize the new feature image to have values between 0 and 255
        new_feature_image_array -= new_feature_image_array.min()
        new_feature_image_array /= new_feature_image_array.max()
        new_feature_image_array *= 255
        # Convert to unsigned 8-bit integer
        new_feature_image_array = new_feature_image_array.astype(np.uint8)
        
        # Convert back to an image
        new_feature_image = Image.fromarray(new_feature_image_array)
