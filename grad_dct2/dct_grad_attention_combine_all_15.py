...
class FeatureFusionNetwork(nn.Module):
    ...
    def forward(self, dct_feature, grad_feature):
        if self.fc1 is None or self.fc2 is None:
            # Compute the flattened size of the attention output features
            input_size = dct_feature.view(dct_feature.size(0), -1).size(1) + grad_feature.view(grad_feature.size(0), -1).size(1)
            self.fc1 = nn.Linear(input_size, 128).to(dct_feature.device)
            self.fc2 = nn.Linear(128, self.output_size).to(dct_feature.device)

        combined = torch.cat((dct_feature.view(dct_feature.size(0), -1), 
                              grad_feature.view(grad_feature.size(0), -1)), dim=1)
...
class ImageProcessor(nn.Module):
    def __init__(self, attention_heads=4, embed_size=32):
        ...
        self.fusion_network = FeatureFusionNetwork(embed_size)
    ...
    def forward(self, image):
        ...
        #print(f"Shape of image_dct: {image_dct.size()}")
        #print(f"Shape of image_grad: {image_grad.size()}")

        B, C, H, W = image_dct.size()
        image_dct_flat = image_dct.view(B, C, H * W).transpose(1, 2) # Shape: [B, H*W, C]
        image_grad_flat = image_grad.view(B, 1, H * W).transpose(1, 2) # Shape: [B, H*W, 1]
        ...
        #print(f"Shape of image_dct_att: {image_dct_att.size()}")
        #print(f"Shape of image_grad_att: {image_grad_att.size()}")
        # 使用FeatureFusionNetwork进行特征融合
        combined_feature = self.fusion_network(image_dct_att, image_grad_att)

    # 修改此部分以正确处理尺寸转换
    # 您可能需要根据您的模型的具体需求调整目标尺寸
        B, _ = combined_feature.size()
        target_H, target_W = H, W  # 根据您的模型需求，这里可能需要调整
        target_C = self.output_size // (target_H * target_W)  # 计算目标通道数
        if target_C * target_H * target_W == self.output_size:
            combined_feature = combined_feature.view(B, target_C, target_H, target_W)
        else:
        # 如果尺寸不匹配，您可能需要进一步调整您的模型结构或输出尺寸
            raise ValueError("Incompatible sizes for reshaping combined_feature")
...
class ProGAN_Dataset(Dataset):
    def __init__(self, root_dir, transform=None):
        ...
        self.image_processor = ImageProcessor(attention_heads=4, embed_size=32)
...
class BasicBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.use_attention:
            ...
            out = out.view(out.shape[0], out.shape[2], int(sqrt(out.shape[1])), int(sqrt(out.shape[1])))
...
def main():
    ...
    # 创建数据加载器
    trainloader = DataLoader(train_dataset, batch_size=128, shuffle=True, num_workers=1, pin_memory=True)
    valloader = DataLoader(val_dataset, batch_size=128, shuffle=False, num_workers=1, pin_memory=True)
