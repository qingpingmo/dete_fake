...
class FusionCapsNet_ViT(nn.Module):
    def __init__(self):
        super(FusionCapsNet_ViT, self).__init__()
        ...
        self.vit = ViT(
            ...
            heads = 16,
            mlp_dim = 2048,
            channels = 512,  # Adjusted for concatenated features
            dropout = 0.1,
            emb_dropout = 0.1
        )

    def forward(self, x_dct, x_grad):
        #print(f"x_dct shape: {x_dct.shape}")
        ...
        #print(f"combined_features shape before reshape: {combined_features.shape}")
        # Ensure that combined_features is reshaped correctly for ViT input
        # ViT expects input of shape [batch_size, channels, height, width]
        # Assuming the ViT model can accept the number of channels in combined_features
        # Print the shape of combined features after reshaping
        
        outputs = self.vit(combined_features)
        return outputs



def ResNet18():
    return FusionCapsNet_ViT()
