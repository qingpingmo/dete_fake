...
class ProGAN_Dataset(Dataset):
    def __init__(self, root_dir, transform=None, feature_dir=None, is_validation=False):
        self.root_dir = root_dir
        self.transform = transform
        self.feature_dir = feature_dir if feature_dir is not None else os.path.join(self.root_dir, 'new_features')
        self.is_validation = is_validation  # Add an attribute to determine if it's a validation dataset
    ...
    def __getitem__(self, idx):
        ...
        image = Image.open(img_path).convert('RGB')  # Ensure original image is RGB

        # Process for new feature image
        feature_path = self._get_feature_path(img_path)
        if os.path.exists(feature_path):
            try:
                new_feature_image = Image.open(feature_path).convert('RGB')  # Ensure new feature image is RGB
        ...
        # Ensure new feature image is RGB (if it's not already)
        if new_feature_image.mode != 'RGB':
            new_feature_image = new_feature_image.convert('RGB')
    ...
    def _process_image(self, image):
        image_array = np.array(image)
        new_feature_image_array = np.zeros_like(image_array, dtype=np.float32)
        ...
        for channel in range(3):  # Process each color channel
            ...
            for i in range(height):
                for j in range(width):
                    neighborhood = channel_data[max(i-1, 0):min(i+2, height), max(j-1, 0):min(j+2, width)]
                    mean_neighborhood = np.mean(neighborhood)
                    std_neighborhood = np.std(neighborhood)
                    
                    # Calculate the correlation coefficient for the pixel's neighborhood
                    if std_neighborhood > 0:
                        correlation_coefficient = (neighborhood - mean_neighborhood) / std_neighborhood
                        correlation_coefficient = np.mean(correlation_coefficient)
                    else:
                        correlation_coefficient = 0  # If the standard deviation is 0, correlation is set to 0
                    
                    # Store the correlation coefficient as the new feature
                    new_feature_image_array[i, j, channel] = correlation_coefficient
        
        # Normalize the new feature image to be in the range [0, 255]
        new_feature_image_array = 255 * (new_feature_image_array - np.min(new_feature_image_array)) / (np.max(new_feature_image_array) - np.min(new_feature_image_array))
...
def main():
    ...
    # 定义待验证的数据集文件夹列表
        ...
        for folder in folders:
            ...
            # 创建验证数据集实例
            val_dataset = ProGAN_Dataset(root_dir=val_dataset_folder, transform=transform, is_validation=True)
