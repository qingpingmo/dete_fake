...
class FPN(nn.Module):
    def __init__(self, input_channels, feature_sizes=[256, 256]):
        super(FPN, self).__init__()
        self.input_channels = input_channels
        self.feature_sizes = feature_sizes

        self.reduce_layers = nn.ModuleList([
            nn.Conv2d(in_channels, fs, kernel_size=1)
            for in_channels, fs in zip(input_channels, feature_sizes)
        ])
        self.smooth_layers = nn.ModuleList([
            nn.Conv2d(fs, fs, kernel_size=3, padding=1)
        ...
        # Adaptive pooling layer to adjust spatial dimensions
        self.adaptive_pool = nn.AdaptiveAvgPool2d((32, 32))

    def forward(self, inputs):
        inputs = [
            reduce_layer(x)
            for x, reduce_layer in zip(inputs, self.reduce_layers)
        ]

        upsampled = inputs[-1]
        for i in range(len(inputs) - 2, -1, -1):
            upsampled = self.upsample_layers[i](upsampled)

            # Adjust the spatial dimensions to match the target feature map
            if upsampled.size()[2:] != inputs[i].size()[2:]:
                upsampled = self.adaptive_pool(upsampled)

            inputs[i] = inputs[i] + upsampled

        features = [
            smooth_layer(x)
            for x, smooth_layer in zip(inputs, self.smooth_layers)
        ]
...
class FusionCapsNetWithFPN(nn.Module):
    def __init__(self):
        ...
        self.fpn = FPN(input_channels=[256, 256], feature_sizes=[256, 256])
        self.global_avg_pool = nn.AdaptiveAvgPool2d(1)
        self.classifier = nn.Linear(256, 2)  # 修改此处
    ...
    def forward(self, x_dct, x_grad):
        ...
        #print(f"dct_features shape: {dct_features.shape}")  # 打印点1
        #print(f"grad_features shape: {grad_features.shape}")  # 打印点1

        combined_features = torch.cat([dct_features, grad_features], dim=1)
        combined_features = self.cbam(combined_features)
        #print(f"combined_features shape after CBAM: {combined_features.shape}")  # 打印点2

        fused_feature = self.fpn([dct_features, grad_features])
        #print(f"fused_feature shape after FPN: {fused_feature.shape}")  # 打印点3

        pooled_feature = self.global_avg_pool(fused_feature)
        #print(f"pooled_feature shape after GlobalAvgPool: {pooled_feature.shape}")  # 打印点4

        flat_feature = pooled_feature.view(x_dct.size(0), -1)
        #print(f"flat_feature shape before classifier: {flat_feature.shape}")  # 打印点5

        outputs = self.classifier(flat_feature)
        return outputs
