...
class FusionCapsNet(nn.Module):
    def __init__(self):
        ...
        self.cbam = CBAM(in_planes=512 + 12 )  # Assuming FAD output has 12 channels, update accordingly
        self.capsule_layer = CapsuleLayer(
            num_capsules=10,
            num_route_nodes=-1,
            in_channels=512+12,  # Update if necessary
            out_channels=32,
            kernel_size=None,
            stride=None
        )
        self.classifier = nn.Linear(288000, 2)  # Adjust size if necessary

    def forward(self, dct_images, grad_images):
        # Apply FAD head on dct_images to get frequency attention features
        fad_features = self.fad_head(dct_images)
        
        # Extract DCT and Gradient features
        dct_features = self.dct_feature_extractor(dct_images)
        grad_features = self.grad_feature_extractor(grad_images)

        # Adjust the size of fad_features to match dct_features and grad_features
        target_size = dct_features.size()[2:]  # Assuming all feature maps have the same spatial dimensions
        fad_features_adjusted = F.interpolate(fad_features, size=target_size, mode='bilinear', align_corners=False)

        # Concatenate all features
        combined_features = torch.cat([fad_features_adjusted, dct_features, grad_features], dim=1)

        # Apply CBAM and subsequent layers
        combined_features = self.cbam(combined_features)
        capsule_output = self.capsule_layer(combined_features)
        outputs = self.classifier(capsule_output.view(dct_images.size(0), -1))
        return outputs
