...
class BasicBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.dropout(out)  # 在ReLU后应用Dropout
        if self.use_attention:
...
optimizer = optim.SGD(net.parameters(), lr=0.1, momentum=0.9, weight_decay=1e-4)  # 已经添加了权重衰减
...
def main():
    ...
    loss_function = nn.CrossEntropyLoss()
    optimizer = optim.SGD(net.parameters(), lr=0.1, momentum=0.9, weight_decay=5e-4)  # 增加了权重衰减
