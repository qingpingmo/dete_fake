...
def apply_wavelet_transform(image, mode='haar', level=1):
    coeffs2 = pywt.dwt2(image, mode)
    LL, (LH, HL, HH) = coeffs2
    return LL
...
class CBAM(nn.Module):
    def __init__(self, in_planes=768, ratio=16, kernel_size=7):  # 修改这里的in_planes为768
...
class ProGAN_Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # 应用小波变换
        image_np = np.array(image)
        wavelet_image = apply_wavelet_transform(image_np[:, :, 0])  # 只取红色通道进行小波变换
        wavelet_image = Image.fromarray((wavelet_image * 255).astype(np.uint8))  # 转换回图像格式

        if self.transform:
            dct_image = self.transform(dct_image)
            grad_image = self.transform(grad_image)
            wavelet_image = self.transform(wavelet_image)  # 应用相同的转换

        label = self.labels[idx]
        return dct_image, grad_image, wavelet_image, label  # 返回包含小波变换图像的元组
...
class FusionCapsNet(nn.Module):
    def __init__(self):
        ...
        self.wavelet_feature_extractor = nn.Sequential(
            nn.Conv2d(1, 64, kernel_size=7, stride=2, padding=3),
            nn.ReLU(),
            nn.MaxPool2d(kernel_size=3, stride=2, padding=1),
            nn.Conv2d(64, 128, kernel_size=5, stride=2, padding=2),
    ...
    def forward(self, x_dct, x_grad,x_wavelet):
        #print(f"Input DCT shape: {x_dct.shape}")  # 打印DCT图像的形状
        #print(f"Input Grad shape: {x_grad.shape}")  # 打印梯度图像的形状
        
        dct_features = self.dct_feature_extractor(x_dct)
        #print(f"DCT Features shape: {dct_features.shape}")  # 打印DCT特征的形状
        
        grad_features = self.grad_feature_extractor(x_grad)
        #print(f"Grad Features shape: {grad_features.shape}")  # 打印梯度特征的形状
        
        wavelet_features = self.wavelet_feature_extractor(x_wavelet) 
        #print(f"Grad Features shape: {grad_features.shape}")

        combined_features = torch.cat([dct_features, grad_features,wavelet_features], dim=1)
        #print(f"Combined Features shape: {combined_features.shape}")  # 打印组合特征的形状

        combined_features = self.cbam(combined_features)
        #print(f"Features after CBAM shape: {combined_features.shape}")  # 打印CBAM处理后的形状
        
        capsule_output = self.capsule_layer(combined_features)
        #print(f"Capsule Output shape: {capsule_output.shape}")  # 打印胶囊层输出的形状
        
        capsule_output = capsule_output.view(x_dct.size(0), -1)
        #print(f"Capsule Output Flattened shape: {capsule_output.shape}")  # 打印扁平化后的形状
        
        outputs = self.classifier(capsule_output)
        #print(f"Final Output shape: {outputs.shape}")  # 打印最终输出的形状
        return outputs
...
def train(trainloader, model, criterion, optimizer, epoch, scheduler, warm):
    ...
    for i, (dct_images, grad_images, wavelet_images, labels) in enumerate(trainloader):
        dct_images, grad_images, wavelet_images, labels = dct_images.to(device), grad_images.to(device), wavelet_images.to(device), labels.to(device)
        
        optimizer.zero_grad()
        outputs = model(dct_images, grad_images, wavelet_images)  # 确保这里传入了三种图像
    ...
    print(f'Epoch {epoch}: Train Loss: {train_loss:.3f}, Train Acc: {acc:.3f}%')
...
def validate(val_loader, model, criterion):
    ...
    with torch.no_grad():
        for i, (dct_images, grad_images, wavelet_images, labels) in enumerate(val_loader):
            dct_images, grad_images, wavelet_images, labels = dct_images.to(device), grad_images.to(device), wavelet_images.to(device), labels.to(device)
            
            outputs = model(dct_images, grad_images, wavelet_images)  # 同样确保这里传入了三种图像
    ...
    print(f'Validation Loss: {val_loss:.3f}, Validation Acc: {acc:.3f}%, Validation AP: {ap:.3f}')
