...
def generate_gradient_image(image_tensor):
    ...
    # Sobel算子扩展到批量和通道
    sobel_x = sobel_x.view(1, 1, 3, 3).repeat(image_tensor.size(1), 1, 1, 1)
    sobel_y = sobel_y.view(1, 1, 3, 3).repeat(image_tensor.size(1), 1, 1, 1)

    # 使用深度学习框架的卷积操作代替convolve2d
...
class FeatureExtractor(nn.Module):
    ...
    def forward(self, x):
        ...
        # 应用DCT变换（确保apply_dct可以处理批量数据）
        dct_features = torch.stack([apply_dct(x[i]) for i in range(batch_size)])
        
        # 应用梯度变换
        gradient_features = generate_gradient_image(x)
        
        # 拼接特征
        features = torch.cat((dct_features, gradient_features), dim=1)
        return features


class ImageFeatureTransformer(nn.Module):
    def __init__(self, feature_size, num_layers, heads):
    ...
    def forward(self, src):
        # Transformer需要src的形状是[S, N, E]，其中S是序列长度，N是批大小，E是特征数
        src = src.permute(1, 0, 2)  # 调整src的形状以满足Transformer的输入要求
        output = self.transformer(src, None)
        return output.permute(1, 0, 2)  # 将输出调整回[N, S, E]的形状


class DCTGradientClassifier(nn.Module):
    def __init__(self):
        super(DCTGradientClassifier, self).__init__()
        self.feature_extractor = FeatureExtractor()
        self.feature_transformer = ImageFeatureTransformer(feature_size=512, num_layers=4, heads=8)
        self.classifier = nn.Linear(512, 2)  # 假设Transformer输出的特征大小为512

    def forward(self, x):
        features = self.feature_extractor(x)
        transformed_features = self.feature_transformer(features)
        # 使用Transformer最后一个时间步的特征进行分类
        output = self.classifier(transformed_features[:, -1, :])
        return output
