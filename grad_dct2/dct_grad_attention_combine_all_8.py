...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1, use_attention=False, heads=4):
        ...
        if self.use_attention:
            # Use TransformerEncoderLayer for self-attention
            self.attention = nn.TransformerEncoderLayer(
                d_model=planes,
                nhead=heads,
                dim_feedforward=planes * 4,  # Adjust as needed
    ...
    def forward(self, x):
        ...
        if self.use_attention:
            ...
            out = out.view(out.shape[0], out.shape[1], -1).permute(2, 0, 1)
            out = self.attention(out)
            # Reshape back to the original shape
            out = out.permute(1, 2, 0).view(out.shape[1], out.shape[2], -1)
