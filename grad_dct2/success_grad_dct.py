...
def generate_gradient_image(image):
    ...
    # 归一化处理保留三通道
    grad_min = np.min(grad, axis=(0, 1), keepdims=True)
    grad_max = np.max(grad, axis=(0, 1), keepdims=True)
    grad_normalized = (grad - grad_min) / (grad_max - grad_min)
    grad_uint8 = (grad_normalized * 255).astype(np.uint8)

    return Image.fromarray(grad_uint8)



def generate_gradient_array(image_slice):
    # image_slice is a 2D array representing a single color channel of an image
    sobel_x = np.array([[-1, 0, 1], [-2, 0, 2], [-1, 0, 1]])
    sobel_y = np.array([[-1, -2, -1], [0, 0, 0], [1, 2, 1]])
    
    grad_x = np.abs(convolve2d(image_slice, sobel_x, mode='same', boundary='wrap'))
    grad_y = np.abs(convolve2d(image_slice, sobel_y, mode='same', boundary='wrap'))
...
def apply_dct_to_array(image_np):
    dct_transformed = np.zeros_like(image_np)
    for i in range(3):  # Apply DCT to each color channel
        dct_transformed[:, :, i] = dct(dct(image_np[:, :, i], axis=0, norm='ortho'), axis=1, norm='ortho')
...
class ProGAN_Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        grad_image = generate_gradient_image(image)  # 生成图像的梯度图(现在是三通道)
...
class FusionCapsNet(nn.Module):
    def __init__(self):
        ...
        # Define a similar feature extractor for grad_dct_features
        self.grad_dct_feature_extractor = nn.Sequential(
            nn.Conv2d(3, 64, kernel_size=7, stride=2, padding=3),
            nn.ReLU(),
            nn.MaxPool2d(kernel_size=3, stride=2, padding=1),
            nn.Conv2d(64, 128, kernel_size=5, stride=2, padding=2),
    ...
    def forward(self, x_dct, x_grad):
        ...
        # Apply the new feature extractor to grad_dct_features
        grad_dct_features = torch.empty_like(dct_features)  # Placeholder tensor with the same shape as dct_features

        # Process each image in the batch individually
        for b in range(x_grad.size(0)):
            grad_image = np.zeros((x_grad.size(2), x_grad.size(3), 3))  # Initialize a NumPy array to hold the gradient image
            for c in range(3):  # Process each channel
                grad_channel = generate_gradient_array(x_grad[b, c].cpu().numpy())
                grad_dct_channel = apply_dct_to_array(grad_channel)
                grad_image[:, :, c] = grad_dct_channel
            
            # Convert the processed grad_image to a tensor and apply the feature extractor
            grad_dct_tensor = torch.tensor(grad_image).to(device).float().permute(2, 0, 1).unsqueeze(0)  # Add a batch dimension
            grad_dct_features[b] = self.grad_dct_feature_extractor(grad_dct_tensor).squeeze(0)  # Remove the batch dimension

        combined_features = torch.cat([dct_features, grad_dct_features], dim=1)
