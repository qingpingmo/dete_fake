...
class ProGAN_Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        diff_arrays = [np.diff(np.array(image)[:, :, channel], axis=0) for channel in range(3)]
        diff_image = np.stack(diff_arrays, axis=-1)
        diff_image = Image.fromarray(np.uint8(diff_image + 128))  # Normalize and convert to uint8
        ...
        return dct_image, diff_image, label  # 返回DCT图像、梯度图和标签
...
class FusionCapsNet(nn.Module):
    ...
    def forward(self, x_dct, x_grad):
        ...
        grad_features = self.diff_feature_extractor(x_grad)
