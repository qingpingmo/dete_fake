...
class DualAttentionModule(nn.Module):
    def __init__(self, in_channels, reduction_ratio=16):
        ...
        self.dct_attention = MultiHeadSelfAttention(self.inter_channels, heads=4)
        self.grad_attention = MultiHeadSelfAttention(self.inter_channels, heads=4)

        self.dct_transform = nn.Sequential(
        ...
        # 修改fusion_transform以接受2个通道的输入
        self.fusion_transform = nn.Sequential(
            nn.Conv2d(2, in_channels, 1, bias=False),  # 注意这里的改变，现在接受2个通道的输入
            nn.BatchNorm2d(in_channels),
            nn.ReLU(inplace=True)
        )

    def forward(self, dct_feat, grad_feat):
        # 需要将2D特征扩展到4D以匹配卷积层的期望输入
        # 假设特征的形状是 [batch_size, feature_dim]
        # 我们需要将其变为 [batch_size, feature_dim, 1, 1]
        #print("Input dct_feat shape:", dct_feat.shape)  # 打印输入的DCT特征形状
        #print("Input grad_feat shape:", grad_feat.shape)  # 打印输入的梯度特征形状
        dct_feat = dct_feat.view(dct_feat.size(0), dct_feat.size(1), 1, 1)
        grad_feat = grad_feat.view(grad_feat.size(0), grad_feat.size(1), 1, 1)

        # Transform features
        dct_feat_transformed = self.dct_transform(dct_feat)
        grad_feat_transformed = self.grad_transform(grad_feat)

        dct_feat_transformed = dct_feat_transformed.view(dct_feat.size(0), 1, -1)
        grad_feat_transformed = grad_feat_transformed.view(grad_feat.size(0), 1, -1)

        # Apply attention
        dct_attended = self.dct_attention(dct_feat_transformed, dct_feat_transformed, dct_feat_transformed, None)
        grad_attended = self.grad_attention(grad_feat_transformed, grad_feat_transformed, grad_feat_transformed, None)

        # Concatenate and transform
        fused_feat = torch.cat([dct_attended, grad_attended], dim=1)
        
        # 在传入fusion_transform之前增加一个额外的维度，以符合卷积层的输入要求
        fused_feat = fused_feat.unsqueeze(-1)  # 将fused_feat的形状从[32, 2, 512]更改为[32, 2, 512, 1]

        fused_feat = self.fusion_transform(fused_feat)

        # 需要将4D特征减少回2D以匹配全连接层的期望输入
...
class MultiHeadSelfAttention(nn.Module):
    ...
    def forward(self, values, keys, query, mask):
        ...
        #print("Values shape:", values.shape)  # 打印values的形状
        #print("Keys shape:", keys.shape)  # 打印keys的形状
        #print("Queries shape:", queries.shape)  # 打印queries的形状
        ...
        out = self.fc_out(out)
        #print("Output shape:", out.shape)  # 打印最终输出的形状
...
class FusionResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=2):
        ...
        self.dual_attention = DualAttentionModule(4194304)
        # 融合层
        self.fusion_layer = nn.Linear(4194304, num_classes)  # 将输入特征数设置为16384
    ...
    # 融合
        out_dct_grad = self.dual_attention(out_dct, out_grad)

    # 确保out_dct_grad是四维的，以适合F.avg_pool2d
        out_dct_grad = out_dct_grad.view(out_dct_grad.size(0), -1, 1, 1)  # 添加一个维度以匹配F.avg_pool2d的期望输入
        print("out_dct_grad shape:", out_dct_grad.shape)

    # 全局平均池化和分类
        out = F.avg_pool2d(out_dct_grad, 1)  # 使用1x1的池化核
