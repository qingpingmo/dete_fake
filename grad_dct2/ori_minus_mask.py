...
def apply_dct_and_randomly_reduce(image):
    image_np = np.array(image)
    # Apply DCT
    dct_transformed = dct(dct(image_np, axis=0, norm='ortho'), axis=1, norm='ortho')
    # Randomly discard 15% of the frequency components
    total_components = dct_transformed.size
    components_to_discard = int(total_components * 0.15)
    flat_indices = np.random.choice(total_components, components_to_discard, replace=False)
    np.put(dct_transformed, flat_indices, 0)  # Set selected components to 0
    # Apply Inverse DCT
    idct_transformed = idct(idct(dct_transformed, axis=1, norm='ortho'), axis=0, norm='ortho')
    # Normalize and convert back to image
    idct_image = np.clip(idct_transformed, 0, 255).astype(np.uint8)
    return Image.fromarray(idct_image)
...
class ProGAN_Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Apply DCT transformation and frequency component reduction
        image_dct_reduced = apply_dct_and_randomly_reduce(image)
        ...
        if os.path.exists(feature_path):
            try:
                ...
                new_feature_image = self._process_image(image)  # Note: using original image here
        ...
        if self.transform:
            image = self.transform(image_dct_reduced)  # Use the DCT-reduced image here
