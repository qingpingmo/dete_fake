    ...
    with torch.no_grad():
        for i, (dct_blocks, grad_blocks, labels) in enumerate(val_loader):
            ...
            for b in range(batch_size):
                ...
                # 迭代每张图片的64个块
                for block_idx in range(64):  # 假设每张图片被分成64个块
                    dct_image = dct_block[block_idx].unsqueeze(0).to(device)  # 添加一个批次维度，并确保在正确的设备上
                    grad_image = grad_block[block_idx].unsqueeze(0).to(device)  # 添加一个批次维度，并确保在正确的设备上
                ...
                correct += (votes.item() == 64).sum().item()  # 如果所有块都预测正确，则整张图片预测正确
                total += 1
