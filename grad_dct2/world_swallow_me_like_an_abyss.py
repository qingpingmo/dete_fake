...
class FeatureFusionNetwork(nn.Module):
    def __init__(self, input_channels, output_channels):
        ...
        self.conv1 = nn.Conv2d(input_channels, output_channels // 2, kernel_size=1, stride=1, padding=0)
        self.bn1 = nn.BatchNorm2d(output_channels // 2)
...
class FusionCapsNet(nn.Module):
    def __init__(self):
        ...
        self.feature_fusion = FeatureFusionNetwork(input_channels=2368, output_channels=512)
        # Adjust the CBAM initialization to expect 2368 input channels
        self.cbam = CBAM(in_planes=512, ratio=16, kernel_size=7)

        # Corrected CapsuleLayer instantiation (removed num_route_nodes argument)
        self.capsule_layer = CapsuleLayer(
            num_capsules=10,
            in_channels=512,  # Adjusted according to combined feature map channels
            out_channels=32,
    ...
    def forward(self, x, x_dct, x_grad):
        ...
        # 应用CBAM和Capsule Network
        cbam_features = self.cbam(fused_features)
