...
class FusionCapsNet(nn.Module):
    def __init__(self):
        ...
        # 注意：需要根据dct_feature_extractor的输出尺寸调整capsule_layer的输入尺寸
        self.capsule_layer = CapsuleLayer(
            num_capsules=10,
            num_route_nodes=-1,
            in_channels=256, 
            out_channels=32,
            kernel_size=3,  # 指定kernel_size
            stride=1       # 指定stride
        )

        # 注意：分类器的输入大小需要根据capsule_layer的输出调整
        self.classifier = nn.Linear(32 * 10 * 32 * 32, 2)  # 可能需要根据实际输出大小调整
...
def train(trainloader, model, criterion, optimizer, epoch, scheduler, warm):
    ...
    for i, (dct_images, labels) in enumerate(trainloader):  # 修改这里
        dct_images, labels = dct_images.to(device), labels.to(device)
        optimizer.zero_grad()

        outputs = model(dct_images)  # 修改这里，移除grad_images
...
def validate(val_loader, model, criterion):
    ...
    with torch.no_grad():
        for i, (dct_images, labels) in enumerate(val_loader):  # 修改这里
            dct_images, labels = dct_images.to(device), labels.to(device)
            outputs = model(dct_images)  # 修改这里，移除grad_images
            loss = criterion(outputs, labels)
