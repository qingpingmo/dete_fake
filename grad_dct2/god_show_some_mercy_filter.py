...
class FADHead(nn.Module):
    def __init__(self, size=256):
        super(FADHead, self).__init__()
        # Initialize DCT matrix
        self.DCT_all = nn.Parameter(torch.tensor(DCT_mat(size)).float(), requires_grad=False)
        self.DCT_all_T = nn.Parameter(torch.transpose(torch.tensor(DCT_mat(size)).float(), 0, 1), requires_grad=False)

        # Define base filters and learnable filters for low, middle, high, and all frequencies
        self.low_filter = Filter(size, 0, size // 16, use_learnable=True, norm=True)
        self.middle_filter = Filter(size, size // 16, size // 8, use_learnable=True, norm=True)
        self.high_filter = Filter(size, size // 8, size, use_learnable=True, norm=True)
        self.all_filter = Filter(size, 0, size * 2, use_learnable=True, norm=True)

    def forward(self, x):
        # Apply DCT to convert image to frequency domain
        x_freq = self.DCT_all @ x @ self.DCT_all_T

        # Apply filters to get different frequency components
        y_low = self.DCT_all_T @ self.low_filter(x_freq) @ self.DCT_all
        y_middle = self.DCT_all_T @ self.middle_filter(x_freq) @ self.DCT_all
        y_high = self.DCT_all_T @ self.high_filter(x_freq) @ self.DCT_all
        y_all = self.DCT_all_T @ self.all_filter(x_freq) @ self.DCT_all

        # Concatenate filtered components in channel dimension
        y_concat = torch.cat([y_low, y_middle, y_high, y_all], dim=1)
        return y_concat


class BlurAndJPEGTransform:
    def __init__(self, probability=0.5, blur_radius=1, jpeg_quality_lower=30, jpeg_quality_upper=100):
        self.probability = probability
        self.blur_radius = blur_radius
        self.jpeg_quality_lower = jpeg_quality_lower
        self.jpeg_quality_upper = jpeg_quality_upper

    def __call__(self, img):
        if random.random() < self.probability:
            img = img.filter(ImageFilter.GaussianBlur(self.blur_radius))
            jpeg_quality = random.randint(self.jpeg_quality_lower, self.jpeg_quality_upper)
            with io.BytesIO() as buffer:
                img.save(buffer, format='JPEG', quality=jpeg_quality)
                buffer.seek(0)
                img = Image.open(buffer)
                img.load()  # This forces the image to be loaded, so the buffer can safely close.
        return img
...
class ProGAN_Dataset(Dataset):
    def __init__(self, root_dir, transform=None, is_train=True):
        ...
        self.data_augmentation = BlurAndJPEGTransform() if is_train else None
        self.image_paths = []
        self.labels = []
    ...
    def __getitem__(self, idx):
        ...
        if self.is_train and self.data_augmentation is not None:
            image = self.data_augmentation(image)  # 仅在训练时应用数据增强
        ...
        if self.transform:
            ...
            dct_image = self.transform(dct_image) # 应用变换到DCT图像
            grad_image = self.transform(grad_image)  # 应用变换到梯度图像

        label = self.labels[idx]
        return image, dct_image, grad_image, label  # 返回包括原始图像、DCT和梯度图像的元组
...
class FusionCapsNet(nn.Module):
    def __init__(self):
        ...
        self.fad_head = FADHead(size=256) 
        ...
        self.cbam = CBAM(in_planes=2368+12, ratio=16, kernel_size=7)

        # Corrected CapsuleLayer instantiation (removed num_route_nodes argument)
        self.capsule_layer = CapsuleLayer(
            num_capsules=10,
            in_channels=2368+12,  # Adjusted according to combined feature map channels
            out_channels=32,
    ...
    def forward(self, x, x_dct, x_grad):
        ...
        resnet_features = self.feature_pooling(resnet_features)  # Ensure standardized size
        #print(f"Shape of pooled ResNet features: {resnet_features.shape}")
        fad_features = self.fad_head(x_dct)
        # Use the DCT feature extractor and apply adaptive pooling
        dct_features = self.dct_feature_extractor(x_dct)
        #print(f"Shape of DCT features: {dct_features.shape}")
        dct_features_pooled = self.feature_pooling(dct_features)
        #print(f"Shape of pooled DCT features: {dct_features_pooled.shape}")

        # Use the gradient feature extractor and apply adaptive pooling
        grad_features = self.grad_feature_extractor(x_grad)
        #print(f"Shape of gradient features: {grad_features.shape}")
        grad_features_pooled = self.feature_pooling(grad_features)
        #print(f"Shape of pooled gradient features: {grad_features_pooled.shape}")


        # Adjust the size of fad_features to match dct_features and grad_features
        target_size = resnet_features.size()[2:]  # Use the size of resnet_features as the target
        fad_features_adjusted = F.interpolate(fad_features, size=target_size, mode='bilinear', align_corners=False)
        dct_features_pooled = F.interpolate(dct_features_pooled, size=target_size, mode='bilinear', align_corners=False)
        grad_features_pooled = F.interpolate(grad_features_pooled, size=target_size, mode='bilinear', align_corners=False)

        # Concatenate along the channel dimension
        combined_features = torch.cat([fad_features_adjusted, resnet_features, dct_features_pooled, grad_features_pooled], dim=1)


        # Apply CBAM for attention mechanism
        cbam_features = self.cbam(combined_features)
        #print(f"Shape of CBAM features: {cbam_features.shape}")

        # Process features through the Capsule Network layer
        capsule_output = self.capsule_layer(cbam_features)
        #print(f"Shape of capsule output: {capsule_output.shape}")
        capsule_output = capsule_output.view(x.size(0), -1)  # Flatten for the classifier
        #print(f"Shape of flattened capsule output: {capsule_output.shape}")

        # Classify using the final classifier
        outputs = self.classifier(capsule_output)
        #print(f"Shape of final output: {outputs.shape}")
...
def main():
    train_dataset = ProGAN_Dataset(root_dir='/opt/data/private/wangjuntong/datasets/progan_train', transform=transform, is_train=True)
    ...
    for epoch in range(80):
        ...
        for folder in folders:
            ...
            val_dataset = ProGAN_Dataset(root_dir=val_dataset_folder, transform=transform, is_train=False)
