...
def compute_phase_spectrum(image):
    ...
    # 对每个颜色通道进行DFT
    dft_shifted = np.zeros_like(image_np, dtype=np.complex128)  # 修改此处
    for i in range(3):  # 针对RGB三个通道
        dft = np.fft.fft2(image_np[:, :, i])
        dft_shifted[:, :, i] = np.fft.fftshift(dft)  # 中心化

    # 计算相位谱
    phase_spectrum = np.angle(dft_shifted)

    # 将相位谱映射到0-255范围内
    phase_spectrum_normalized = (phase_spectrum + np.pi) / (2 * np.pi) * 255
    phase_image = phase_spectrum_normalized.astype(np.uint8)
    return Image.fromarray(phase_image)
...
class ProGAN_Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        phase_image = compute_phase_spectrum(image)  # 新增

        if self.transform:
            dct_image = self.transform(dct_image)
            grad_image = self.transform(grad_image)
            phase_image = self.transform(phase_image)  # 新增

        label = self.labels[idx]
        return dct_image, grad_image, phase_image, label  # 更新返回值，包含相位谱
...
class FusionCapsNet(nn.Module):
    def __init__(self):
        ...
        # 修改这里，in_planes设置为512
        self.cbam = CBAM(in_planes=512)  
        
        # 更新这里，in_channels设置为768，因为capsule_input由combined_features和phase_features合并得到
    ...
    def forward(self, x_dct, x_grad, x_phase):
        dct_features = self.dct_feature_extractor(x_dct)
        grad_features = self.grad_feature_extractor(x_grad)
        phase_features = self.phase_feature_extractor(x_phase)

        # 在这里合并dct_features和grad_features，然后通过CBAM
        combined_features = torch.cat([dct_features, grad_features], dim=1)
        combined_features = self.cbam(combined_features)

        # 然后将phase_features与CBAM处理后的combined_features合并，形成capsule_input
        capsule_input = torch.cat([combined_features, phase_features], dim=1)
...
def train(trainloader, model, criterion, optimizer, epoch, scheduler, warm):
    ...
    for i, (dct_images, grad_images, phase_images, labels) in enumerate(trainloader):  # 更新
        dct_images, grad_images, phase_images, labels = dct_images.to(device), grad_images.to(device), phase_images.to(device), labels.to(device)  # 更新
        optimizer.zero_grad()

        outputs = model(dct_images, grad_images, phase_images)
...
def validate(val_loader, model, criterion):
    ...
    with torch.no_grad():
        for i, (dct_images, grad_images, phase_images, labels) in enumerate(val_loader):  # 更新
            dct_images, grad_images, phase_images, labels = dct_images.to(device), grad_images.to(device), phase_images.to(device), labels.to(device)  # 更新
            outputs = model(dct_images, grad_images, phase_images) 
            loss = criterion(outputs, labels)
