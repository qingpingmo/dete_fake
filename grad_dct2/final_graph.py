...
class GraphFusionLayer(nn.Module):
    ...
    def forward(self, x, edge_index):
        # x is the node features, edge_index is the graph connectivity in COO format
...
class FusionCapsNetWithGraphFusion(nn.Module):
    def __init__(self):
        super(FusionCapsNetWithGraphFusion, self).__init__()
        ...
        self.graph_fusion = GraphFusionLayer(in_channels=256, out_channels=256)
    ...
    def forward(self, x_dct, x_grad):
        ...
        # 在图融合模块前准备特征
        combined_features = torch.cat([dct_features, grad_features], dim=1)  # 合并特征
        combined_features = combined_features.unsqueeze(1)  # 增加一个维度以符合图融合模块的输入要求

        # 图融合
        fused_features = self.graph_fusion(combined_features)
...
def ResNet18():
    return FusionCapsNetWithGraphFusion()
