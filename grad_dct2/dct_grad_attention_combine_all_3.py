...
class DualAttentionResNet(nn.Module):
    def __init__(self):
        ...
        # 注意力分支
        self.attention_dct = MultiHeadSelfAttention(embed_size=262144, heads=64)
        self.attention_grad = MultiHeadSelfAttention(embed_size=262144, heads=64)
    ...
    def forward(self, x_dct, x_grad):
        # 应用注意力机制
        x_dct = self.attention_dct(x_dct)
        x_grad = self.attention_grad(x_grad)

        # 特征融合
        x_combined = torch.cat((x_dct, x_grad), dim=1)
        x_combined = x_combined.view(-1, 1, 512, 512)
        # 通过 ResNet 进行分类
        out = self.resnet(x_combined)
        return out
...
class ProGAN_Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.transform:
            ...
            image_dct_gray = self.transform(image_dct_gray)
            image_grad_gray = self.transform(image_grad_gray)

        label = self.labels[idx]
        return image, image_dct_gray, image_grad_gray, label
...
def train(trainloader, model, criterion, optimizer, epoch, scheduler, warm):
    ...
    for i, (image, image_dct, image_grad, labels) in enumerate(trainloader):
        # 将数据移至GPU
        image_dct, image_grad, labels = image_dct.to(device), image_grad.to(device), labels.to(device)
        optimizer.zero_grad()

        # 修改模型的输入
        outputs = model(image_dct, image_grad)
...
def validate(val_loader, model, criterion):
    ...
    with torch.no_grad():
        for batch_idx, (image, image_dct, image_grad, targets) in enumerate(val_loader):
        # 将数据移至GPU
            image_dct, image_grad, targets = image_dct.to(device), image_grad.to(device), targets.to(device)
...
def main():
    ...
    # 实例化模型并将其移动到指定的GPU
    net = DualAttentionResNet().to(device)
