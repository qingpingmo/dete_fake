    ...
    # 将图像均匀切分为8x8个小块
        ...
        for i in range(8):
            for j in range(8):
                ...
                block_gradient = calculate_block_gradient(block)
            
                if block_gradient < min_gradient:
                    min_gradient = block_gradient
                    selected_block = block

        selected_image = Image.fromarray(selected_block)

    # 应用DCT变换
        dct_image = apply_dct(selected_image)

    # 应用梯度变换
        grad_image = generate_gradient_image(selected_image)
