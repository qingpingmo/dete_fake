...
class ScaledDotProductAttention(nn.Module):
    ...
    def forward(self, query, key, value, mask):
        ...
        attention = torch.einsum('nqhd,nkhd->nhqk', [query, key]) / self.scale
...
class MultiHeadSelfAttention(nn.Module):
    def __init__(self, embed_size, heads, input_dim):
        ...
        self.values = nn.Linear(self.input_dim, self.head_dim * heads, bias=False)
        self.keys = nn.Linear(self.input_dim, self.head_dim * heads, bias=False)
        self.queries = nn.Linear(self.input_dim, self.head_dim * heads, bias=False)
