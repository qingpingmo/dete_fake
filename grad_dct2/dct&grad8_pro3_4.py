...
class NonLocalBlock(nn.Module):
    def __init__(self, in_channels, inter_channels=None, dimension=2, mode='embedded', sub_sample=False):
        super(NonLocalBlock, self).__init__()

        assert dimension in [1, 2, 3]
        assert mode in ['embedded', 'gaussian', 'concatenate']
        ...
        # 现在可以安全地使用 self.inter_channels
        self.g = conv_nd(in_channels=in_channels, out_channels=self.inter_channels, kernel_size=1)  # 注意这里的改动

        if mode == 'embedded':
            self.theta = conv_nd(in_channels=in_channels, out_channels=self.inter_channels, kernel_size=1, stride=1, padding=0)
            self.phi = conv_nd(in_channels=in_channels, out_channels=self.inter_channels, kernel_size=1, stride=1, padding=0)
        elif mode == 'gaussian':
            self.theta = None
            self.phi = None
            ...
            self.theta = None
            self.phi = None
            raise NotImplementedError('Concatenate mode has not been implemented.')

        self.W = conv_nd(in_channels=self.inter_channels, out_channels=in_channels, kernel_size=1, stride=1, padding=0)
        self.W_bn = bn(in_channels)

        if sub_sample:
            self.g = nn.Sequential(self.g, max_pool(kernel_size=2))
            if self.theta is not None:
                self.theta = nn.Sequential(self.theta, max_pool(kernel_size=2))
...
class FusionCapsNet(nn.Module):
    def __init__(self):
        ...
        # Non-local 注意力模块，这里修正了输入通道数为512，中间通道数为256
        self.non_local_block = NonLocalBlock(in_channels=512, inter_channels=256)  # 注意这里的改动

        # Capsule层
        self.capsule_layer = CapsuleLayer(
            num_capsules=10,
            num_route_nodes=-1,
            in_channels=512,  # 更新通道数以匹配Non-local注意力后的结果
            out_channels=32,
            kernel_size=None,
            stride=None
        )
        ...
        # 分类器
        self.classifier = nn.Linear(288000, 2)  # 根据capsule_output的展平版本调整
    ...
    def forward(self, x_dct, x_grad):
        ...
        # Non-local注意力机制
        combined_features = self.non_local_block(combined_features)
