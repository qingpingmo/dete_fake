...
def apply_zca_whitening(image_np):
    # 将图像标准化
    scaler = StandardScaler().fit(image_np)
    image_np_normalized = scaler.transform(image_np)

    # 计算协方差矩阵
    cov_matrix = np.cov(image_np_normalized, rowvar=False)
    ...
    # 应用ZCA白化
    image_zca_whitened = np.dot(image_np_normalized, zca_matrix.T)
    return scaler.inverse_transform(image_zca_whitened)

def apply_pca_reduction(image_np, n_components=0.95):
    pca = PCA(n_components=n_components)
    pca.fit(image_np)
    image_pca_reduced = pca.transform(image_np)
    return pca.inverse_transform(image_pca_reduced)
...
def preprocess_image(image):
    ...
    # 检查图像是否为灰度图像，并在必要时将其转换为3通道图像
    if len(image_np.shape) == 2:
        image_np = np.stack((image_np,)*3, axis=-1)

    # 应用ZCA白化和PCA降维
    image_zca_whitened = apply_zca_whitening(image_np.reshape(-1, 3)).reshape(image_np.shape)
    image_pca_reduced = apply_pca_reduction(image_zca_whitened.reshape(-1, 3), n_components=0.95).reshape(image_np.shape)

    # 转换回PIL图像以进行后续的图像变换
    return Image.fromarray(np.uint8(image_pca_reduced))
...
class ProGAN_Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # 对DCT和梯度图像应用ZCA白化和PCA降维
        dct_image_np = np.array(dct_image)
        grad_image_np = np.array(grad_image)

        dct_image_zca_whitened = apply_zca_whitening(dct_image_np.reshape(-1, dct_image_np.shape[-1])).reshape(dct_image_np.shape)
        grad_image_zca_whitened = apply_zca_whitening(grad_image_np.reshape(-1, grad_image_np.shape[-1])).reshape(grad_image_np.shape)

        dct_image_pca_reduced = apply_pca_reduction(dct_image_zca_whitened.reshape(-1, dct_image_zca_whitened.shape[-1]), n_components=0.95).reshape(dct_image_zca_whitened.shape)
        grad_image_pca_reduced = apply_pca_reduction(grad_image_zca_whitened.reshape(-1, grad_image_zca_whitened.shape[-1]), n_components=0.95).reshape(grad_image_zca_whitened.shape)

        # 将处理后的NumPy数组转换回PIL图像
        dct_image_processed = Image.fromarray(np.uint8(dct_image_pca_reduced))
        grad_image_processed = Image.fromarray(np.uint8(grad_image_pca_reduced))

        # 应用其他转换（如缩放、归一化）
        if self.transform:
            dct_image_processed = self.transform(dct_image_processed)
            grad_image_processed = self.transform(grad_image_processed)

        # 确定图像标签
        label = 1 if '1_fake' in img_path else 0

        return dct_image_processed, grad_image_processed, label
...
class FusionCapsNet(nn.Module):
    def __init__(self):
       ...
       # Capsule层
        ...
        # 分类器
        self.classifier = nn.Linear(288000, 2)  # 更新为与capsule_output的展平版本相匹配
...
def main():
    ...
    # 创建数据加载器
    trainloader = DataLoader(train_dataset, batch_size=256, shuffle=True, num_workers=5, pin_memory=True)
    valloader = DataLoader(val_dataset, batch_size=256, shuffle=False, num_workers=5, pin_memory=True)
