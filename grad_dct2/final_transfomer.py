...
class TransformerEncoderLayer(nn.Module):
    def __init__(self, dim, heads, dim_feedforward=2048, dropout=0.1):
...
class VisionTransformer(nn.Module):
    def __init__(self, img_size=256, patch_size=16, num_classes=2, dim=1024, depth=6, heads=8, mlp_dim=2048):
        super(VisionTransformer, self).__init__()
        self.num_patches = (img_size // patch_size) ** 2
        self.patch_size = patch_size
        self.dim = dim

        # 注意这里调整了输入特征数量以匹配新的通道数
        self.patch_to_embedding = nn.Linear(4 * patch_size * patch_size, dim)
        self.pos_embedding = nn.Parameter(torch.randn(1, self.num_patches + 1, dim))
        self.cls_token = nn.Parameter(torch.randn(1, 1, dim))
        self.transformer = nn.Sequential(*[TransformerEncoderLayer(dim, heads) for _ in range(depth)])
    ...
    def forward(self, x_dct, x_grad):
        x = torch.cat([x_dct, x_grad], dim=1)  # 结果形状为[N, 4, H, W]
        x = self.patchify(x)
    ...
    def patchify(self, imgs):
        ...
        patches = imgs.reshape(batch_size, channels, height // p, p, width // p, p)
        patches = patches.permute(0, 2, 4, 1, 3, 5).flatten(1, 2)
        patches = patches.flatten(2, -1)  # 确保正确地将每个patch拉平
        return patches
...
def main():
    ...
    # 初始化模型
    net = VisionTransformer(img_size=256, patch_size=32, num_classes=2, dim=1024, depth=6, heads=8, mlp_dim=2048).to(device)
