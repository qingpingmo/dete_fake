    ...
    def forward(self, x):
        if self.num_route_nodes != -1:
            ...
            for i in range(self.num_iterations):
                ...
                outputs = self.squash((probs * priors).sum(dim=2, keepdim=True), dim=-1)
...
class SimplifiedCapsuleLayer(nn.Module):
    def __init__(self, num_capsules, in_channels, out_channels, kernel_size=3, stride=1):
    ...
    def forward(self, x):
        ...
        outputs = outputs.view(x.size(0), -1, outputs.size(-1))  # Combine all capsules
        return outputs

# Replace the original CapsuleLayer in the FusionCapsNet class with SimplifiedCapsuleLayer
class FusionCapsNet(nn.Module):
    def __init__(self):
        ...
        self.dct_feature_extractor = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Conv2d(128, 8, kernel_size=3, stride=1, padding=1),
            nn.ReLU(),
        )
        ...
        # Using the simplified CapsuleLayer
        self.capsule_layer = SimplifiedCapsuleLayer(
            num_capsules=8,  # Reduced number of capsules for simplicity
            in_channels=16,  # Combined feature channels from dct and grad feature extractors
            out_channels=32,  # Output channels for each capsule
            kernel_size=3,  # Default kernel size
            stride=1  # Default stride
        )

        # The feature_transform layer might need adjustment based on the output size of the simplified capsule layer
        # This requires a manual calculation or a dummy forward pass to determine the exact size
        self.feature_transform = nn.Linear(327680, 288000)  # Placeholder, adjust based on actual output size
        self.classifier = nn.Linear(288000, 2)  # Final classification layer
    ...
    def forward(self, x_dct, x_grad):
        ...
        capsule_output = capsule_output.view(x_dct.size(0), -1)  # Flatten the capsule outputs
        features_transformed = self.feature_transform(capsule_output)  # Transform features to match classifier input
        outputs = self.classifier(features_transformed)
        return outputs
