...
def train(trainloader, model, criterion, optimizer, epoch, scheduler, warm, data_augmentation='mixup'):
    model.train()
    running_loss = 0.0
    correct = 0
    total = 0
    ...
    for i, (dct_images, grad_images, labels) in enumerate(trainloader):
        ...
        # Apply Mixup or CutMix separately on dct_images and grad_images
        if data_augmentation == 'mixup':
            mixed_dct, targets_a, targets_b, lam = mixup_data(dct_images, labels)
            mixed_grad, _, _, _ = mixup_data(grad_images, labels)  # We use the same lam and targets
        elif data_augmentation == 'cutmix':
            mixed_dct, targets_a, targets_b, lam = cutmix_data(dct_images, labels)
            mixed_grad, _, _, _ = cutmix_data(grad_images, labels)  # We use the same lam and targets
        else:
            mixed_dct, mixed_grad = dct_images, grad_images
            targets_a, targets_b, lam = labels, labels, 1

        optimizer.zero_grad()
        # Pass both mixed_dct and mixed_grad as separate arguments
        outputs = model(mixed_dct, mixed_grad)
