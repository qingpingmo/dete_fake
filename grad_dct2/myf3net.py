...
# 定义数据转换
transform = Compose([
    Resize((299, 299)),  # 调整图像大小以匹配 F3Net 输入
    ToTensor(),
...
# 初始化模型
net = F3Net(num_classes=2, img_width=299, img_height=299, LFS_window_size=10, LFS_M=6).to(device)
...
def train(trainloader, model, criterion, optimizer, epoch, scheduler, warm):
    ...
    for i, (images, labels) in enumerate(trainloader):  # 修改这里
        images, labels = images.to(device), labels.to(device)
        optimizer.zero_grad()

        outputs = model(images)  # 修改这里，只传入images
...
def validate(val_loader, model, criterion):
    ...
    with torch.no_grad():
        for i, (images, labels) in enumerate(val_loader):  # Corrected this line
            images, labels = images.to(device), labels.to(device)
            outputs = model(images)  # Adjusted this line, only images are passed to the model
            loss = criterion(outputs, labels)
...
def main():
    ...
    optimizer = optim.SGD(net.parameters(), lr=0.1, momentum=0.9, weight_decay=1e-4)
    train_scheduler = optim.lr_scheduler.MultiStepLR(optimizer, milestones=[80, 120], gamma=0.1)
    warmup_scheduler = WarmUpLR(optimizer, iter_per_epoch * len(trainloader))
    best_acc = 0

    # 训练循环
    for epoch in range(80):
        train_loss, train_acc = train(trainloader, net, loss_function, optimizer, epoch, warmup_scheduler, 1)
        if epoch >= 1:
            train_scheduler.step()
        print(f'Epoch: {epoch}, Train Loss: {train_loss:.3f}, Train Acc: {train_acc:.3f}')

        # 验证循环
        for folder in ['progan', 'stylegan', 'stylegan2', 'biggan', 'cyclegan', 'stargan', 'gaugan', 'deepfake']:
