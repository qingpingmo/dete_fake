...
def fcanet50(num_classes, pretrained=False):
...
class FusionCapsNet(nn.Module):
    def __init__(self):
        ...
        self.adjusted_fcanet = fcanet50(num_classes=2, pretrained=False)  # Adjusted to use the entire FcaNet model

        self.capsule_layer = CapsuleLayer(
            num_capsules=10,
            num_route_nodes=-1,
            in_channels=256,  # Adjusted according to the output channels of the last Conv layer before capsule layer
            out_channels=32,
            kernel_size=(9, 9),  # Adjust these dimensions based on the output of your adjusted_fcanet
            stride=1,
        )

        self.classifier = nn.Linear(32 * 10, 2)  # Adjusted based on the output of the capsule layer
    ...
    def forward(self, x_dct, x_grad):
        ...
        # Here, you would apply any necessary transformations to ensure the combined_features
        # are compatible with self.adjusted_fcanet's expected input shape

        adjusted_features = self.adjusted_fcanet(combined_features)
        
        # Ensure adjusted_features is reshaped or pooled to match the expected input shape for the capsule layer
        # Example: adjusted_features = F.adaptive_avg_pool2d(adjusted_features, (9, 9))

        capsule_output = self.capsule_layer(adjusted_features)
        capsule_output = capsule_output.view(x_dct.size(0), -1)  # Ensure capsule_output is flattened properly
