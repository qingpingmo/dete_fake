...
def train(trainloader, model, criterion, optimizer, epoch, scheduler, warm):
    ...
    for i, (images, dct_images, grad_images, labels) in enumerate(trainloader):  # Correctly unpack all items
        dct_images, grad_images, labels = dct_images.to(device), grad_images.to(device), labels.to(device)
        optimizer.zero_grad()

        # You may want to use the `images` here as well, depending on your network's architecture
...
def validate(val_loader, model, criterion):
    ...
    with torch.no_grad():
        for i, (images, dct_images, grad_images, labels) in enumerate(val_loader):  # Correctly unpack all items
            dct_images, grad_images, labels = dct_images.to(device), grad_images.to(device), labels.to(device)
            outputs = model(dct_images, grad_images)
            loss = criterion(outputs, labels)
