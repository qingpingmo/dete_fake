...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1, use_attention=False, heads=4, use_se=False):
...
class FusionCapsNet(nn.Module):
    def __init__(self):
        ...
        self.dct_feature_extractor = nn.Sequential(
            BasicBlock(3, 64, stride=2, use_se=True),
            BasicBlock(64, 128, stride=2, use_se=True),
        )
        
        # Gradient feature extractor
        self.grad_feature_extractor = nn.Sequential(
            BasicBlock(1, 64, stride=2, use_se=True),
            BasicBlock(64, 128, stride=2, use_se=True),
        )
        ...
        self.capsule_layer = CapsuleLayer(
            num_capsules=10, 
            num_route_nodes=-1, 
            in_channels=256,  # Updated for combined features
            out_channels=32, 
            kernel_size=None, 
            stride=None
        )
        ...
        # Classifier
        self.classifier = nn.Linear(288000, 2)  # Updated for flattened capsule output

    def forward(self, x_dct, x_grad):
        # Extract features from DCT and gradient images
        ...
        # Dynamic routing in Capsule Layer
        capsule_output = self.capsule_layer(combined_features)
        capsule_output = capsule_output.view(x_dct.size(0), -1)  # Flatten capsule output
