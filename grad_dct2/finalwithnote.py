...
class SELayer(nn.Module):
    def __init__(self, channel, reduction=16):
        ...
        self.fc = nn.Sequential(
            ...
            nn.Linear(channel // reduction, channel, bias=False),  # 全连接层，升维
            nn.Sigmoid()  # Sigmoid激活函数
        )

    def forward(self, x):
        b, c, _, _ = x.size()  # 获取输入的尺寸
        y = self.avg_pool(x).view(b, c)  # 平均池化并重塑
        y = self.fc(y).view(b, c, 1, 1)  # 通过全连接层并重塑
        return x * y.expand_as(x)  # 返回缩放的输入
...
# 定义MultiHeadSelfAttention类，实现多头自注意力机制
class MultiHeadSelfAttention(nn.Module):
    def __init__(self, embed_size, heads):
        super(MultiHeadSelfAttention, self).__init__()
        self.embed_size = embed_size  # 嵌入向量大小
        self.heads = heads  # 头的数量
        self.head_dim = embed_size // heads  # 每个头的维度
        assert (
            self.head_dim * heads == embed_size
        ), "Embedding size needs to be divisible by heads"  # 确保嵌入向量大小可以被头的数量整除
        self.values = nn.Linear(self.head_dim, self.head_dim, bias=False)  # 值向量的线性变换
        self.keys = nn.Linear(self.head_dim, self.head_dim, bias=False)  # 键向量的线性变换
        self.queries = nn.Linear(self.head_dim, self.head_dim, bias=False)  # 查询向量的线性变换
        self.fc_out = nn.Linear(heads * self.head_dim, embed_size)  # 最后的线性变换

    def forward(self, values, keys, query, mask):
        N = query.shape[0]  # 批次大小
        value_len, key_len, query_len = values.shape[1], keys.shape[1], query.shape[1]  # 值、键和查询的长度
        values = values.reshape(N, value_len, self.heads, self.head_dim)  # 重塑值向量
        keys = keys.reshape(N, key_len, self.heads, self.head_dim)  # 重塑键向量
        queries = query.reshape(N, query_len, self.heads, self.head_dim)  # 重塑查询向量
        values = self.values(values)  # 应用值向量的线性变换
        keys = self.keys(keys)  # 应用键向量的线性变换
        queries = self.queries(queries)  # 应用查询向量的线性变换
        energy = torch.einsum("nqhd,nkhd->nhqk", [queries, keys])  # 计算能量分数
        if mask is not None:
            energy = energy.masked_fill(mask == 0, float("-1e20"))  # 应用掩码

        attention = torch.softmax(energy / (self.embed_size ** (1 / 2)), dim=3)  # 计算注意力权重
        out = torch.einsum("nhql,nlhd->nqhd", [attention, values]).reshape(
            N, query_len, self.heads * self.head_dim
        )  # 计算输出
        out = self.fc_out(out)  # 应用最后的线性变换
        return out  # 返回多头自注意力的输出

# 定义AttentionLayer类，实现注意力机制
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1, use_attention=False, heads=4, attention_type='se'):
        ...
        self.conv1 = nn.Conv2d(in_planes, planes, kernel_size=3, stride=stride, padding=1, bias=False)  # 第一个卷积层
        self.bn1 = nn.BatchNorm2d(planes)  # 第一个批归一化层
        self.conv2 = nn.Conv2d(planes, planes, kernel_size=3, stride=1, padding=1, bias=False)  # 第二个卷积层
        self.bn2 = nn.BatchNorm2d(planes)  # 第二个批归一化层
        self.shortcut = nn.Sequential()  # 定义残差连接
        if stride != 1 or in_planes != self.expansion * planes:  # 如果需要调整输入和输出的维度
            self.shortcut = nn.Sequential(
                nn.Conv2d(in_planes, self.expansion * planes, kernel_size=1, stride=stride, bias=False),  # 1x1卷积用于调整维度
                nn.BatchNorm2d(self.expansion * planes)  # 批归一化
            )

        self.use_attention = use_attention  # 是否使用注意力机制
        self.attention_type = attention_type  # 注意力机制的类型
        if self.use_attention:  # 如果使用注意力机制
            if self.attention_type == 'se':  # 如果是SE注意力机制
                self.attention = SELayer(planes)  # 创建SE注意力层
            elif self.attention_type == 'mhsa':  # 如果是多头自注意力机制
                self.attention = MultiHeadSelfAttention(planes, heads)  # 创建多头自注意力层
            else:
                raise NotImplementedError("Attention type not supported")  # 如果指定的注意力机制类型不支持，抛出异常

    def forward(self, x):
        identity = x  # 保存输入作为恒等映射
        out = F.relu(self.bn1(self.conv1(x)))  # 应用第一个卷积层和批归一化层，然后使用ReLU激活函数
        out = self.bn2(self.conv2(out))  # 应用第二个卷积层和批归一化层
        out += self.shortcut(x)  # 添加残差连接

        if self.use_attention:  # 如果使用注意力机制
            out = self.attention(out)  # 通过注意力层
...
class FusionCapsNet(nn.Module):
    ...
    def forward(self, x_dct, x_grad):
        dct_features = self.dct_feature_extractor(x_dct)  # 提取DCT特征
        grad_features = self.grad_feature_extractor(x_grad)  # 提取梯度特征
        combined_features = torch.cat([dct_features, grad_features], dim=1)  # 在通道维度上拼接DCT和梯度特征
        combined_features = self.cbam(combined_features)  # 应用CBAM注意力模块
        capsule_output = self.capsule_layer(combined_features)  # 通过胶囊层
        capsule_output = capsule_output.view(x_dct.size(0), -1)  # 重塑胶囊层输出
        outputs = self.classifier(capsule_output)  # 通过分类器
