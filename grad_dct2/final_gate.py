...
class GatedFusion(nn.Module):
    def __init__(self, input_channels, feature_size=256):
        super(GatedFusion, self).__init__()
        self.input_channels = input_channels
        self.feature_size = feature_size

        self.conv1x1 = nn.Conv2d(sum(input_channels), feature_size, kernel_size=1)
        self.gate = nn.Sequential(
            nn.Conv2d(sum(input_channels), feature_size, kernel_size=1),
            nn.Sigmoid()
        )
    ...
    def forward(self, inputs):
        ...
        gated_features = self.conv1x1(x) * self.gate(x)
...
class FusionCapsNetWithGatedFusion(nn.Module):
    def __init__(self):
        super(FusionCapsNetWithGatedFusion, self).__init__()
        ...
        self.cbam = CBAM(in_planes=512) 
         ...
         # Gated Fusion module
        self.gated_fusion = GatedFusion(input_channels=[256, 256])

        # Global Average Pooling
        self.global_avg_pool = nn.AdaptiveAvgPool2d(1)

        # Classifier
        self.classifier = nn.Linear(256, 2)  # Adjusted for the output of GatedFusion
    ...
    def forward(self, x_dct, x_grad):
        ...
        # Gated Fusion
        fused_features = self.gated_fusion([dct_features, grad_features])
        pooled_features = self.global_avg_pool(fused_features)
        flat_features = pooled_features.view(x_dct.size(0), -1)

        outputs = self.classifier(flat_features)
        return outputs

def ResNet18():
    return FusionCapsNetWithGatedFusion()
