...
def apply_dct(image, keep_ratio=0.1):
    ...
    # 标准化到0-255并转换为uint8，添加1e-8以避免除以零
...
def generate_gradient_image(image):
    ...
    gradient_normalized = (grad - np.min(grad)) / (np.max(grad) - np.min(grad) + 1e-8)
    ...
    def _load_images(self, current_dir):
        for item in os.listdir(current_dir):
            ...
            if os.path.isdir(path):
                # 如果folders被指定且当前文件夹不在列表中，则跳过
                if self.folders is not None and item not in self.folders:
                    continue
...
class FusionCapsNet(nn.Module):
    def __init__(self):
        ...
        # CBAM模块
        self.cbam = CBAM(in_planes=256)  # 假设融合特征的通道数为256

        # Capsule层
        self.capsule_layer = CapsuleLayer(
            num_capsules=10, 
            num_route_nodes=-1, 
            in_channels=256,  # 与融合特征通道数匹配
            out_channels=32, 
            kernel_size=None, 
            stride=None
        )
        ...
        # 分类器
        self.classifier = nn.Linear(1230080, 2)  # 根据capsule_output的实际形状调整
    ...
    if len(val_loader) > 0:
        ...
        if len(all_outputs) > 0:
            ...
            if all_outputs.ndim == 1:
                all_outputs = np.expand_dims(all_outputs, axis=0)

            probs = torch.nn.functional.softmax(torch.tensor(all_outputs), dim=1).numpy()
            positive_class_probs = probs[:, 1]
            ap = average_precision_score(all_targets, positive_class_probs)
        else:
            ap = 0  # 如果all_outputs为空，则AP设置为0
    else:
        val_loss = 0
        acc = 0
        ap = 0  # 如果val_loader为空，则损失、准确率和AP都设置为0
...
def main():
    ...
    for folder in folders:
        print(f"\nValidating on {folder} dataset")
        val_dataset = ProGAN_Dataset(root_dir=f'/root/rstao/datasets/CNN_synth_testset/{folder}', transform=transform, folders=[folder])
        ...
        for epoch in range(160):
            ...
            #print(f'Epoch: {epoch}, Folder: {folder}, Train Loss: {train_loss:.3f}, Train Acc: {train_acc:.3f}, Val Loss: {val_loss:.3f}, Val Acc: {val_acc:.3f}')
            print(f'Epoch: {epoch}, Folder: {folder}, Train Loss: {train_loss:.3f}, Train Acc: {train_acc:.3f}, Val Loss: {val_loss:.3f}, Val Acc: {val_acc:.3f}, Val AP: {val_ap:.3f}')
