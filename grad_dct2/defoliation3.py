...
class ProGAN_Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        orig_image = image  # 使用原始图像

        if self.transform:
            dct_image = self.transform(dct_image)
            grad_image = self.transform(grad_image)
            orig_image = self.transform(orig_image)  # 应用转换到原始图像

        label = self.labels[idx]
        return dct_image, grad_image, orig_image, label
...
class FusionCapsNet(nn.Module):
    def __init__(self):
        ...
        self.dct_cbam = CBAM(in_planes=256)
        self.dct_residual_blocks = nn.Sequential(
        ...
        self.grad_cbam = CBAM(in_planes=256)
        self.grad_residual_blocks = nn.Sequential(
            ResidualBlock(256, 256),
            ResidualBlock(256, 256)
        )

        self.orig_feature_extractor = nn.Sequential(
            nn.Conv2d(3, 64, kernel_size=7, stride=2, padding=3),
            nn.ReLU(),
            nn.MaxPool2d(kernel_size=3, stride=2, padding=1),
            nn.Conv2d(64, 128, kernel_size=5, stride=2, padding=2),
        ...
        self.orig_cbam = CBAM(in_planes=256)
        self.orig_residual_blocks = nn.Sequential(
            ResidualBlock(256, 256),
            ResidualBlock(256, 256)
        )
        ...
        self.capsule_layer = CapsuleLayer(
            num_capsules=10,
            num_route_nodes=-1,
            in_channels=768,  # 3个特征通道合并后的总通道数
            out_channels=32,
            kernel_size=None,
            stride=None
        )

        self.classifier = nn.Linear(288000, 2)  # 根据您的胶囊网络输出调整

    def forward(self, x_dct, x_grad, x_orig):
        dct_features = self.dct_feature_extractor(x_dct)
        dct_features = self.dct_cbam(dct_features)
        dct_features = self.dct_residual_blocks(dct_features)
        
        grad_features = self.grad_feature_extractor(x_grad)
        grad_features = self.grad_cbam(grad_features)
        grad_features = self.grad_residual_blocks(grad_features)
        
        orig_features = self.orig_feature_extractor(x_orig)
        orig_features = self.orig_cbam(orig_features)
        orig_features = self.orig_residual_blocks(orig_features)
        
        combined_features = torch.cat([dct_features, grad_features, orig_features], dim=1)
...
def train(trainloader, model, criterion, optimizer, epoch, scheduler, warm):
    ...
    for i, (dct_images, grad_images, orig_images, labels) in enumerate(trainloader):
...
def validate(val_loader, model, criterion):
    ...
    with torch.no_grad():
        for i, (dct_images, grad_images, orig_images, labels) in enumerate(val_loader):
            dct_images, grad_images, orig_images, labels = dct_images.to(device), grad_images.to(device), orig_images.to(device), labels.to(device)
            outputs = model(dct_images, grad_images, orig_images)
            loss = criterion(outputs, labels)
