...
output_directory="./our_model_finishedhahahaha_txt"
# 循环遍历两个元素的所有组合
for ((i=0; i<${#models[@]}; i++)); do
    for ((j=i+1; j<${#models[@]}; j++)); do
        ...
        # 检查任务是否已经运行，如果是则跳过
        if [ -e "${output_directory}/completed_${model1}_${model2}_our_model.txt" ]; then
            echo "Task for $model1 and $model2 already completed. Skipping..."
            continue
        fi
        touch "${output_directory}/completed_${model1}_${model2}_our_model.txt"
        # 打印当前组合（可选）
        echo "Training with model: $model1 and $model2 on GPU 3"
