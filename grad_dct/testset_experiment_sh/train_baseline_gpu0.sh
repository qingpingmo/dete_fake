...
for ((i=0; i<${#models[@]}; i++)); do
    for ((j=i+1; j<${#models[@]}; j++)); do
        ...
        CUDA_VISIBLE_DEVICES=0 python ../train2.py --name "baseline_${model1}_${model2}" \
        --blur_prob 0.1 --blur_sig 0.0,3.0 \
        --jpg_prob 0.1 --jpg_method cv2,pil \
        --jpg_qual 30,100 \
        --dataroot /root/rstao/datasets/ \
        --dataset1_root "/root/rstao/datasets/CNN_synth_testset/$model1" \
        --dataset2_root "/root/rstao/datasets/CNN_synth_testset/$model2" \
...
# CUDA_VISIBLE_DEVICES=0 python train2.py --name baseline_progan_stylegan \
# --blur_prob 0.1 --blur_sig 0.0,3.0 \
# --jpg_prob 0.1 --jpg_method cv2,pil \
# --jpg_qual 30,100 \
# --dataroot /opt/data/private/rstao/code/DeepfakeDetection/datasets/CNNDetection/ \
# --dataset1_root /opt/data/private/rstao/code/DeepfakeDetection/datasets/CNNDetection/CNN_synth_testset/progan/ \
# --dataset2_root /opt/data/private/rstao/code/DeepfakeDetection/datasets/CNNDetection/CNN_synth_testset/stylegan/ \
# --classes chair,horse --batch_size 128 --delr_freq 10 --lr 0.0002 --niter 100 \
# --train_split progan_train --val_split progan_val